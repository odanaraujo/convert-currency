...
func (c *Transaction) Valid() error {
	...
	if value := c.validToCurrency(); value == 0 {
...
func (c *Transaction) validFromCurrency(valueCurrency string) int {
	value := 0
	for index, ts := range typeSimbol {
		if string(ts[index]) == value {
			value++
		}
	}
...
func (c *Transaction) validToCurrency() int {
	value := 0
	for index, ts := range typeSimbol {
		if string(ts[index]) == c.ToCurrency {
			value++
		}
	}
